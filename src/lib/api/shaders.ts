...
export async function getInstalledShaders(
  minecraftPath: string,
): Promise<ShaderPack[]> {
  return invoke("get_installed_shaders", { minecraftPath });
}

export async function toggleShader(
  minecraftPath: string,
  shaderFile: string,
  enabled: boolean,
): Promise<void> {
...
export async function getShaderInfo(
  minecraftPath: string,
  shaderFile: string,
): Promise<ShaderPack> {
...
export async function searchModrinthShaders(
  ...
  return invoke("search_modrinth_shaders", {
    query,
    minecraftVersion,
    limit,
    offset,
...
export async function searchModrinthShadersWithFacets(
  ...
  return invoke("search_modrinth_shaders_with_facets", {
    query,
    minecraftVersion,
    facets,
    limit,
...
export async function getModrinthShaderDetails(
  projectId: string,
): Promise<ShaderDownload> {
...
export async function downloadAndInstallShader(
  ...
  return invoke("download_and_install_shader", {
    minecraftPath,
    downloadUrl,
    filename,
  });
...
export async function setupShaderSymlink(
  ...
  return invoke("setup_shader_symlink", {
    minecraftPath,
    dedicatedFolder,
    symlinkName,
  });
...
export async function removeShaderSymlink(
  minecraftPath: string,
  symlinkName: string,
): Promise<void> {
