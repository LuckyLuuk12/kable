...
<div class="skins-container">
  ...
  {#if error}
    <div class="error-msg">
      ...
      {error}
      <button on:click={() => error = ''}><Icon name="close" size="sm" /></button>
    </div>
  {/if}
    ...
    {#if capes.length > 0}
      <section>
        ...
        <div class="capes-grid">
          <button class="cape-card" class:active={!activeCape} on:click={() => handleApplyCape(null)} disabled={loading}>
            <div class="cape-preview no-cape"><Icon name="close" size="md" /></div>
            <h4>No Cape</h4>
            {#if !activeCape}<span class="badge">Active</span>{/if}
          </button>
          {#each capes as cape}
            <button class="cape-card" class:active={SkinsService.isCapeActive(cape)} on:click={() => handleApplyCape(cape.id)} disabled={loading}>
    ...
    <section>
      ...
      {#if accountSkins.length === 0}
        <div class="empty">
          ...
          <button on:click={openUploadDialog}><Icon name="upload" size="sm" />Upload</button>
        </div>
      {:else}
        <div class="skins-grid">
          {#each accountSkins as skin (skin.id)}
            <div class="skin-card" role="button" tabindex="0" class:current={SkinsService.isSkinActive(skin)} on:mouseenter={() => hoveredSkinId = skin.id} on:mouseleave={() => hoveredSkinId = null}>
              <div class="preview">
                {#if skin.url}
                  <SkinViewer3D skinUrl={skin.url} height={180} model={getModel(skin.model)} animation={hoveredSkinId === skin.id ? 'walk' : 'idle'} />
                {:else}
              ...
              <div class="details">
                <div class="title-row">
                  ...
                  {#if SkinsService.isSkinActive(skin)}<span class="badge">Active</span>{/if}
                </div>
                <div class="meta">
                ...
                <div class="actions">
                  {#if !SkinsService.isSkinActive(skin)}
                    <button class="apply" on:click={() => handleApplySkin(skin.id)} disabled={loading}><Icon name="check" size="sm" />Apply</button>
                  {/if}
                  <button on:click={() => openEditModal(skin)} disabled={loading} title="Edit"><Icon name="edit" size="sm" /></button>
                  {#if !SkinsService.isSkinActive(skin)}
                    <button class="danger" on:click={(e) => removeSkin(skin.id, e)} disabled={loading} title="Remove"><Icon name="trash" size="sm" /></button>
                  {/if}
                </div>
              </div>
            </div>
  ...
  <div class="modal-bg" on:click={() => showEditModal = false} on:keypress={(e) => e.key === 'Escape' && (showEditModal = false)} role="button" tabindex="0">
    <div class="modal" on:click|stopPropagation on:keypress={(e) => e.key === 'Escape' && (showEditModal = false)} role="button" tabindex="0">
      <div class="modal-header">
        <h3> Edit Skin</h3>
        <button on:click={() => showEditModal = false}><Icon name="close" size="sm" /></button>
      </div>
      <div class="modal-body">
        <label>Name <input type="text" bind:value={editName} placeholder="Skin name" /></label>
        <label>Cape <select bind:value={editCapeId}><option value="">None</option>{#each capes as c}<option value={c.id}>{SkinsService.getCapeDisplayName(c)}</option>{/each}</select></label>
        <label>Model <div class="radio-group"><label><input type="radio" bind:group={editSlim} value={false} />Classic</label><label><input type="radio" bind:group={editSlim} value={true} />Slim</label></div></label>
      </div>
      <div class="modal-footer">
        <button on:click={() => showEditModal = false}>Cancel</button>
        <button class="primary" on:click={saveEdit} disabled={!editName}><Icon name="check" size="sm" />Save</button>
      </div>
    </div>
  </div>
{/if}
...
{#if showAddModal}
  <div class="modal-bg" on:click={() => showAddModal = false} on:keypress={(e) => e.key === 'Escape' && (showAddModal = false)} role="button" tabindex="0">
    <div class="modal" on:click|stopPropagation on:keypress={(e) => e.key === 'Escape' && (showAddModal = false)} role="button" tabindex="0">
      <div class="modal-header">
        <h3> Upload Skin</h3>
        <button on:click={() => showAddModal = false}><Icon name="close" size="sm" /></button>
      </div>
      <div class="modal-body">
        <label>Name <input type="text" bind:value={addName} placeholder="Skin name" /></label>
        <label>Cape <select bind:value={addCapeId}><option value="">None</option>{#each capes as c}<option value={c.id}>{SkinsService.getCapeDisplayName(c)}</option>{/each}</select></label>
        <label>Model <div class="radio-group"><label><input type="radio" bind:group={addSlim} value={false} />Classic</label><label><input type="radio" bind:group={addSlim} value={true} />Slim</label></div></label>
        <div class="file-info"><Icon name="file" size="sm" />{addFilePath.split(/[\\\\/]/).pop()}</div>
      </div>
      <div class="modal-footer">
        <button on:click={() => showAddModal = false}>Cancel</button>
        <button class="primary" on:click={uploadSkin} disabled={!addName}><Icon name="upload" size="sm" />Upload</button>
      </div>
    </div>
  </div>
{/if}
