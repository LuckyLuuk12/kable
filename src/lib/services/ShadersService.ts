...
export class ShadersService {
  ...
  async loadShaders() {
    ...
    console.log(
      `[ShadersService] Loading shaders (offset: ${offset}, limit: ${limit})`,
    );
    ...
    try {
      ...
      console.log(
        `[ShadersService] Successfully loaded ${shaders.length} shaders`,
      );
      shaderDownloads.set(shaders);
    } catch (e: any) {
      console.error(
        "[ShadersService] Failed to load shaders:",
  ...
  async setFilter(filter: ShaderFilterFacets | null) {
    ...
    console.log(
      "[ShadersService] Setting shader filter:",
  ...
  async prevPage() {
    ...
    console.log(
      `[ShadersService] Moving to previous page (new offset: ${offset})`,
    );
  ...
  async searchShaders(query: string) {
    ...
    try {
      ...
      console.log(
        `[ShadersService] Found ${shaders.length} shaders matching "${query}"`,
      );
      shaderDownloads.set(shaders);
      shadersOffset.set(0); // Reset to first page on new search
    } catch (e: any) {
      console.error(
        "[ShadersService] Failed to search shaders:",
        e.message || "Unknown error",
      );
  ...
  async downloadShader(
    ...
    console.log(
      `[ShadersService] Downloading shader "${shader.name}" to ${isGlobal ? "global" : installation?.name || "unknown"}`,
    );
    ...
    try {
      if (isGlobal) {
        ...
        throw new Error(
          "No installation selected for dedicated shader installation",
        );
      }
      console.log(
  ...
  static async searchShaders(
    ...
    return ShadersAPI.searchModrinthShaders(
      query,
      minecraftVersion,
      limit,
      offset,
  ...
  static async downloadShaderGlobal(shader: ShaderDownload): Promise<string> {
    ...
    const filename =
      shader.download_url.split("/").pop() || `${shader.name}.zip`;
  ...
  static filterShaderDownloads(
    shaders: ShaderDownload[],
