  ...
  async function importFromMinecraftFolder() {
    try {
      ...
      if (path) {
        ...
        showNotification('Installations imported successfully from .minecraft folder!', 'success');
      }
    } catch (error) {
      const errorMsg = error instanceof Error ? error.message : 'Unknown error';
      console.error('Failed to import from .minecraft folder:', error);
      showNotification(`Failed to import from .minecraft folder: ${errorMsg}`, 'error');
    } finally {
      isImporting = false;
    }
  }
...
<div class="installations-page">
  ...
  {#if notificationMessage}
    <div class="notification notification-{notificationType}">
      <Icon name={notificationType === 'success' ? 'check-circle' : 'alert'} size="sm" />
      <span>{notificationMessage}</span>
      <button class="close-btn" on:click={() => { notificationMessage = ''; notificationType = ''; }}>
        <Icon name="x" size="sm" />
      </button>
    </div>
  {/if}
...
<style lang="scss">
  ...
  .notification {
    ...
    border-radius: var(--border-radius);
    box-shadow: 0 0.5rem 2rem rgba(0, 0, 0, 0.2), 0 0.25rem 0.5rem rgba(0, 0, 0, 0.1);
