...
use crate::skins::types::{AccountCape, PlayerProfile};
use crate::skins::types::{AccountSkin, CurrentSkin, SkinModel};
...
pub async fn get_current_skin_info() -> Result<CurrentSkin, String> {
    Logger::console_log(LogLevel::Info, "🔍 Fetching current skin information", None);
...
pub async fn get_all_account_skins() -> Result<Vec<AccountSkin>, String> {
    ...
    Logger::console_log(
        LogLevel::Info,
        &format!(
            "📊 Found {} total skins ({} online, {} local)",
            all_skins.len(),
...
pub async fn get_local_skins() -> Result<Vec<AccountSkin>, String> {
    ...
    if !custom_skins_file.exists() {
        Logger::console_log(
            LogLevel::Debug,
            &format!(
                "📁 No custom skins file found at: {}",
                custom_skins_file.display()
            ),
            None,
        );
        return Ok(Vec::new());
    ...
    Logger::console_log(
        LogLevel::Debug,
        &format!(
            "📄 Found {} local skins in launcher_custom_skins.json",
            local_skins.len()
        ),
        None,
    );
...
pub async fn convert_custom_skin_to_account_skin(
    key: String,
    skin_entry: super::CustomSkinEntry,
) -> Result<AccountSkin, String> {
    // Determine model from 'slim' field
    let model = if skin_entry.slim {
        SkinModel::Slim
    } else {
        SkinModel::Classic
    };
    ...
    for (img_data, img_type) in [
        (&skin_entry.skin_image, "skin"),
        (&skin_entry.model_image, "model"),
    ] {
        ...
        if is_base64_png(img_data) {
            ...
            Logger::console_log(
                LogLevel::Debug,
                &format!(
                    "Returning data URL for skin '{}', type '{}', length {}",
                    key,
                    img_type,
                    b64.len()
                ),
