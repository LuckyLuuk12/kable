use crate::constants::{CONFIG_DIR, PROFILES_FILE, VERSIONS_DIR};
...
pub async fn read_kable_profiles() -> Result<Vec<KableProfile>, String> {
    ...
    let data = async_fs::read_to_string(&path)
        .await
        .map_err(|e| format!("Failed to read {}: {}", PROFILES_FILE, e))?;
    let mut installations = task::spawn_blocking(move || {
        serde_json::from_str::<Vec<KableProfile>>(&data)
            .map_err(|e| format!("Failed to parse {}: {}", PROFILES_FILE, e))
    })
    .await
    .unwrap()?;
    ...
    for installation in installations.iter_mut() {
        if installation.dedicated_config_folder.is_none() {
            installation.dedicated_config_folder =
                Some(format!("{}/{}", CONFIG_DIR, installation.id));
            needs_update = true;
        }
    }
