use crate::constants::{VERSIONS_DIR, LIBRARIES_DIR, fabric_profile_url, fabric_profile_jar_url};
...
impl Launchable for FabricLaunchable {
    async fn prepare(&self, context: &LaunchContext) -> Result<(), String> {
        ...
        if !fabric_json.exists() || !fabric_jar.exists() {
            let version_data = get_version(version_id.clone()).await
                .ok_or_else(|| format!("Could not find version data for {}", version_id))?;
            let extra = &version_data.extra;
            let mc_version = extra.get("minecraft_version").and_then(|v| v.as_str())
                .ok_or("No 'minecraft_version' in version.extra")?;
            let fabric_version = extra.get("version").and_then(|v| v.as_str())
                .ok_or("No 'version' in version.extra")?;

            crate::system::fs::ensure_folder(&versions_dir.join(version_id))
                .map_err(|e| format!("Failed to create version dir: {e}"))?;

            let profile_url = fabric_profile_url(mc_version, fabric_version);
            
            let client = Client::new();
            let profile_json_str = client.get(&profile_url).send().await
                .map_err(|e| format!("Failed to download Fabric profile: {e}"))?
                .text().await
                .map_err(|e| format!("Failed to read Fabric profile: {e}"))?;

            std::fs::write(&fabric_json, profile_json_str)
                .map_err(|e| format!("Failed to save Fabric manifest: {e}"))?;
            ...
            std::fs::write(&fabric_jar, jar_bytes)
                .map_err(|e| format!("Failed to save Fabric JAR: {e}"))?;
        }
        ...
        let inherited_id = manifest.get("inheritsFrom").and_then(|v| v.as_str())
            .ok_or("Fabric manifest must inherit from a vanilla version")?;
        ...
        ensure_assets_for_manifest(&context.minecraft_dir, &vanilla_manifest, AssetMode::MinimalWithSounds, Some(&context.installation.id)).await?;
    ...
    async fn launch(&self, context: &LaunchContext) -> Result<LaunchResult, String> {
        ...
        let libraries_path = PathBuf::from(&context.minecraft_dir).join(LIBRARIES_DIR);
        let version_jar_path = PathBuf::from(&context.minecraft_dir).join(VERSIONS_DIR).join(version_id).join(format!("{}.jar", version_id));
        
        let classpath = build_classpath_from_manifest_with_instance(&merged_manifest, &libraries_path, &version_jar_path, Some(&context.installation.id));
        ...
        let (mut jvm_args_vec, game_args_vec) = build_jvm_and_game_args_with_instance(&merged_manifest, &variables, Some(&context.installation.id));

        for (k, v) in &context.installation.parameters_map {
            if k.starts_with("--") {
                jvm_args_vec.push(k.clone());
                if !v.is_empty() { jvm_args_vec.push(v.clone()); }
            }
        }
