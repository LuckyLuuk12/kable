...
impl Launchable for FabricLaunchable {
    async fn prepare(&self, context: &LaunchContext) -> Result<(), String> {
        ...
        let manifest_struct =
            load_and_merge_fabric_manifest_struct(&context.minecraft_dir, version_id)?;
    ...
    async fn launch(&self, context: &LaunchContext) -> Result<LaunchResult, String> {
        ...
        let manifest_struct =
            load_and_merge_fabric_manifest_struct(&context.minecraft_dir, version_id)?;
        let inherited_id = manifest_struct
            .inherits_from
            .as_ref()
            .ok_or("Missing inheritsFrom")?;
        ...
        let variables = build_variable_map(
            context,
            Some(&merged_manifest),
            &classpath,
            Some(&context.installation.parameters_map),
        );
        let (mut jvm_args_vec, game_args_vec) = build_jvm_and_game_args_with_instance(
            &merged_manifest,
            &variables,
            Some(&context.installation.id),
        );
