...
fn feature_matches(features: &std::collections::HashMap<String, bool>, context: &RuleContext) -> bool {
    for (feature, required) in features {
        let actual = match feature.as_str() {
            ...
            _ => {
                // Unknown features must not accidentally match.
                false
            }
        };
...
async fn replace_variables(value: &str, profile: &KableProfile, manifest: &McVersionManifest, classpath: &str) -> Result<String, String> {
    let mc_root = crate::system::fs::mc_dir()?;
    ...
    let logs_root = mc_root.join(crate::constants::LOGS_DIR);
    ...
    result = result.replace("${path}", &logs_root.to_string_lossy());
    ...
    if let Some(account) = active_account {
         ...
         * NOTE:
         *
         * Do not assume that clientid/clientId is the Minecraft UUID.
         *
         * If your account model contains the actual Microsoft/Minecraft
         * client ID, use that here instead.
         *
         * This is left as the current value until that field is available
...
async fn resolve_arg(
    ...
    match arg {
        Arg::String(value) => Ok(vec![replace_variables(value, profile, manifest, classpath).await?]),
        ...
        Arg::Rule(RuleArg { rules, value }) => {
            ...
            match value {
                StringOrList::Single(value) => Ok(vec![replace_variables(value, profile, manifest, classpath).await?]),

                StringOrList::List(values) => {
                    let mut resolved = Vec::with_capacity(values.len());

                    for value in values {
                        resolved.push(replace_variables(value, profile, manifest, classpath).await?);
                    }
...
async fn resolve_arguments(
    ...
    for arg in args {
        resolved.extend(resolve_arg(arg, profile, manifest, classpath, context).await?);
    }
    ...
    match arguments {
        Arguments::Structured(arguments) => {
            ...
            if let Some(jvm) = &arguments.jvm {
                args.extend(resolve_arguments(jvm, profile, manifest, classpath, context).await?);
            }

            if let Some(default_user_jvm) = &arguments.default_user_jvm {
                args.extend(resolve_arguments(default_user_jvm, profile, manifest, classpath, context).await?);
            }

            Ok(args)
        }

        Arguments::Mixed(arguments) => resolve_arguments(arguments, profile, manifest, classpath, context).await,

        Arguments::Flat(arguments) => {
            let mut args = Vec::with_capacity(arguments.len());

            for argument in arguments {
                args.push(replace_variables(argument, profile, manifest, classpath).await?);
            }
    ...
    match arguments {
        Arguments::Structured(arguments) => {
            ...
            if let Some(game) = &arguments.game {
                args.extend(resolve_arguments(game, profile, manifest, classpath, context).await?);
            }
...
async fn build_command(manifest: &McVersionManifest, profile: &KableProfile) -> Result<Command, String> {
    ...
    /*
     * This is the launch context used when evaluating the
     * Minecraft manifest's conditional arguments.
     *
     * These values should eventually come from the actual
     * profile/account/launch settings rather than being
     * hard-coded.
     */
    let context = RuleContext {
        ...
        /*
         * Kable currently launches at the primary monitor's
         * resolution, so custom resolution is enabled.
         *
         * If Kable gets a user-configurable resolution mode,
         * this should instead come from that setting.
         */
    ...
    let mut jvm_args = resolve_jvm_arguments(manifest, profile, &classpath, &context).await?;

    let game_args = resolve_game_arguments(manifest, profile, &classpath, &context).await?;
    ...
    if let Some(logging) = &manifest.logging {
        if let Some(client) = &logging.client {
            if let Some(argument) = &client.argument {
                jvm_args.push(replace_variables(argument, profile, manifest, &classpath).await?);
            }
        }
    }
