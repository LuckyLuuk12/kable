...
use self::manager::{ModProvider, ModInfoKind, ModFilter};
use self::modrinth::{ModrinthProvider};
...
static MODRINTH: Lazy<Mutex<ModrinthProvider>> = Lazy::new(|| Mutex::new(ModrinthProvider::default()));

pub fn set_provider_filter(provider: ProviderKind, installation: Option<&KableInstallation>, filter: Option<ModFilter>) {
    match provider {
...
pub async fn download_mod(provider: ProviderKind, mod_id: &str, version_id: Option<&str>, target_dir: &std::path::Path) -> Result<(), String> {
    match provider {
        ProviderKind::Modrinth => {
            let prov = {
                let prov_guard = MODRINTH.lock().unwrap();
