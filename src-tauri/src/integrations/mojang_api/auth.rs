...
pub fn get_client_id() -> Result<String, String> {
    var("AZURE_CLIENT_ID")
        ...
        .map_err(|_: String| "AZURE_CLIENT_ID / CLIENT_ID not set and no fallback available".to_string())
}
...
pub fn get_redirect_uri() -> Result<String, String> {
    var("AZURE_REDIRECT_URI")
        ...
        .map_err(|_: String| "AZURE_REDIRECT_URI / REDIRECT_URI not set and no fallback available".to_string())
}

pub fn get_oauth_port() -> u16 {
    var("OAUTH_PORT").ok().and_then(|v| v.parse().ok()).unwrap_or(DEFAULT_OAUTH_PORT)
}
...
pub async fn refresh_microsoft_token(token: MicrosoftToken) -> Result<MicrosoftToken, String> {
    let refresh_token = token.refresh_token.ok_or_else(|| "No refresh token available".to_string())?;

    let client_id = get_client_id()?;
    let client = BasicClient::new(ClientId::new(client_id))
        .set_auth_uri(AuthUrl::new(MSA_AUTHORIZE_URL.to_string()).map_err(|e| format!("Failed to create auth URL: {}", e))?)
        .set_token_uri(TokenUrl::new(MSA_TOKEN_URL.to_string()).map_err(|e| format!("Failed to create token URL: {}", e))?);
    ...
    let new_expires_at = Utc::now() + chrono::Duration::seconds(token_result.expires_in().map(|d| d.as_secs() as i64).unwrap_or(3600));
