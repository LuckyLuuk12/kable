...
use oauth2::{
    ...
    Scope, StandardDeviceAuthorizationResponse, TokenUrl, TokenResponse
};
...
use crate::auth::auth_util::{open_url, write_launcher_account, LauncherAccount, MinecraftProfile, get_client_id};
...
static DEVICE_AUTH_STORAGE: once_cell::sync::Lazy<Arc<Mutex<HashMap<String, StandardDeviceAuthorizationResponse>>>> = 
...
pub async fn start_microsoft_device_auth() -> Result<DeviceCodeResponse, String> {
    Logger::console_log(LogLevel::Info, "🔐 Starting Microsoft device code authentication...", None);
    ...
    let client = BasicClient::new(
        ...
        AuthUrl::new(MSA_AUTHORIZE_URL.to_string())
            .map_err(|e| {
                Logger::console_log(LogLevel::Error, &format!("❌ Failed to create auth URL: {}", e), None);
                format!("Failed to create auth URL: {}", e)
            })?,
        Some(TokenUrl::new(MSA_TOKEN_URL.to_string())
            .map_err(|e| {
                Logger::console_log(LogLevel::Error, &format!("❌ Failed to create token URL: {}", e), None);
    ...
    .set_device_authorization_url(
        DeviceAuthorizationUrl::new(DEVICE_CODE_URL.to_string())
            .map_err(|e| {
                Logger::console_log(LogLevel::Error, &format!("❌ Failed to create device code URL: {}", e), None);
    ...
    let details: StandardDeviceAuthorizationResponse = client
        ...
        .map_err(|e| {
            Logger::console_log(LogLevel::Error, &format!("❌ Failed to initiate device code flow: {}", e), None);
        ...
        .await
        .map_err(|e| {
            Logger::console_log(LogLevel::Error, &format!("❌ Failed to request device code: {}", e), None);
    ...
    Logger::console_log(
        LogLevel::Debug, 
        &format!("  - Device code length: {} chars", response.device_code.len()), 
        None
    );
    ...
    // Store the device authorization response for polling
    {
        ...
        Logger::console_log(
            LogLevel::Debug, 
            &format!("💾 Stored device auth response for device code: {}...", &response.device_code[..std::cmp::min(8, response.device_code.len())]), 
            None
        );
    }
    ...
    Logger::console_log(
        LogLevel::Info, 
        &format!("🌐 Device code generated. User code: {}", response.user_code), 
        None
    );
    ...
    Logger::console_log(
        LogLevel::Debug, 
        &format!("🌐 Attempting to open verification URL: {}", response.verification_uri), 
        None
    );
...
#[tauri::command]
pub async fn poll_microsoft_device_auth(device_code: String) -> Result<Option<MicrosoftToken>, String> {
    Logger::console_log(LogLevel::Debug, "🔄 Polling Microsoft device authentication...", None);
    
    // Debug: Log device code info
    Logger::console_log(
        LogLevel::Debug, 
        &format!("🔍 Polling with device code: {}...", &device_code[..std::cmp::min(8, device_code.len())]), 
        None
    );
    ...
    let details = match device_auth_response {
        Some(details) => {
            Logger::console_log(LogLevel::Debug, "✅ Found stored device authorization response", None);
            details
        },
        None => {
            Logger::console_log(LogLevel::Error, "❌ Device code not found in storage", None);
            return Err("Device code not found. Please start authentication flow first.".to_string());
        }
    };
        ...
        .await
    {
        Ok(token) => {
            ...
            Logger::console_log(
                LogLevel::Debug, 
                &format!("🕒 Token expires in {} seconds (at {})", expires_in_secs, expires_at), 
                None
            );
            ...
            Logger::console_log(
                LogLevel::Debug, 
                &format!("✅ Microsoft token obtained - length: {} chars", ms_token.access_token.len()), 
                None
            );
            ...
            Logger::console_log(LogLevel::Info, "✅ Microsoft authentication successful!", None);
            Ok(Some(ms_token))
        }
        Err(oauth2::RequestTokenError::ServerResponse(ref err)) => {
            ...
            if err.error().as_ref() == "authorization_pending" {
                ...
                Logger::console_log(LogLevel::Debug, "🐌 Polling too fast, slowing down...", None);
                ...
                let error_desc = err.error_description().map(|s| s.as_str()).unwrap_or("No description");
                Logger::console_log(
                    LogLevel::Error, 
...
#[tauri::command]
pub async fn complete_minecraft_auth(microsoft_token: MicrosoftToken) -> Result<LauncherAccount, String> {
    Logger::console_log(LogLevel::Info, "🎮 Exchanging Microsoft token for Minecraft access...", None);
    
    // Debug: Log the Microsoft token details (without sensitive data)
    Logger::console_log(
        LogLevel::Debug, 
        &format!("🔍 Microsoft token expires at: {}", microsoft_token.expires_at), 
        None
    );
    Logger::console_log(
        LogLevel::Debug, 
        &format!("🔍 Microsoft token length: {} chars", microsoft_token.access_token.len()), 
        None
    );
    ...
    Logger::console_log(LogLevel::Debug, "🔄 Starting Microsoft to Minecraft token exchange...", None);
    ...
    Logger::console_log(
        LogLevel::Debug, 
        &format!("  - Access token length: {} chars", mc_token.access_token().as_ref().len()), 
        None
    );
    ...
    write_launcher_account(account.clone()).await
        .map_err(|e| {
            Logger::console_log(
                LogLevel::Error, 
