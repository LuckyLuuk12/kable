...
pub async fn apply_window_state(window: WebviewWindow, state: WindowState) -> Result<(), String> {
    ...
    let target_monitor = if let (Some(saved_name), Some(saved_pos), Some(saved_size)) = 
        (&state.monitor_name, state.monitor_position, state.monitor_size) {
        
        // Get all available monitors
        let monitors = window.available_monitors().map_err(|e| e.to_string())?;
        
        // First, try to find by exact name match
        let mut found_monitor = monitors.iter().find(|monitor| {
            monitor.name().map(|s| s.as_str()).unwrap_or("") == saved_name
        });
    ...
    if let Some(monitor) = target_monitor {
        ...
        if state.x >= 0 && state.y >= 0 {
            ...
            if let (Some(saved_pos), Some(_saved_size)) = (state.monitor_position, state.monitor_size) {
                ...
                let clamped_x = new_x.max(monitor_pos.x).min(monitor_pos.x + monitor_size.width as i32 - state.width as i32);
                let clamped_y = new_y.max(monitor_pos.y).min(monitor_pos.y + monitor_size.height as i32 - state.height as i32);
                ...
                let monitor_pos = monitor.position();
                let monitor_size = monitor.size();
                let clamped_x = state.x.max(monitor_pos.x).min(monitor_pos.x + monitor_size.width as i32 - state.width as i32);
                let clamped_y = state.y.max(monitor_pos.y).min(monitor_pos.y + monitor_size.height as i32 - state.height as i32);
...
pub async fn get_monitor_info(window: WebviewWindow) -> Result<Vec<serde_json::Value>, String> {
    ...
    let monitor_info: Vec<serde_json::Value> = monitors.iter().map(|monitor| {
        let name = monitor.name().cloned().unwrap_or_else(|| "Unknown".to_string());
