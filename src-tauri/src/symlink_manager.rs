...
async fn read_custom_symlinks() -> Result<CustomSymlinksConfig, String> {
    ...
    if !config_path.exists() {
        return Ok(CustomSymlinksConfig { symlinks: Vec::new() });
    }
    ...
    for entry in entries.flatten() {
        ...
        if path.is_symlink() {
            ...
            if !should_skip_directory(dir_name) {
                scan_directory_for_managed_symlinks(&path, minecraft_root, custom_config, symlinks)?;
            }
        }
    }
...
pub async fn list_all_symlinks() -> Result<Vec<SymlinkInfo>, String> {
    ...
    scan_directory_for_managed_symlinks(&minecraft_dir, &minecraft_dir, &custom_config, &mut symlinks)?;
...
pub async fn toggle_symlink_disabled(destination: String, id: Option<String>) -> Result<bool, String> {
    ...
    if let Some(symlink_id) = id {
        ...
        let symlink = config.symlinks.iter_mut()
...
impl SymlinkManager {
    ...
    pub async fn setup_for_installation(&self, installation_id: &str) -> Result<(), String> {
        ...
        self.cleanup_for_installation_switch(installation_id).await?;
